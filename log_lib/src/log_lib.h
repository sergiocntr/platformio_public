...
// Gestito centralmente tramite SYS_PROFILE in SystemProfiles.h
#ifndef DEBUG_SERIAL_LOG
// Se non definito, la seriale di log è disabilitata (es. Nextion attivo)
#endif

// Abilitazione UDP Log
#ifndef DISABLE_UDP_LOG
...
class UdpLogger {
public:
  UdpLogger() : _port(0), _enabled(true) { _deviceName[0] = '\0'; }
...
#else
#define _SERIAL_LOG(prefix, fmt, ...)                                          \
  LOG_SERIAL_PORT.printf(prefix fmt "\n", ##__VA_ARGS__)
#endif
#else
#define _SERIAL_LOG(prefix, fmt, ...)
#endif
